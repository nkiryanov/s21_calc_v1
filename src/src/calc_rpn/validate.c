...
bool validate_math_expression(calc_expr_t expression) {
  ...
  bool has_error = false;
  char tokens[][10] = {FUNCTIONS, OPERATORS, PARENTHESES, VARIABLES};
  uint32_t count_tokens = COUNT_OF(tokens);
  ...
  for (uint32_t start = 0; start < expression.size && !has_error;) {
    ...
    if (!token_match) {
      token_match = match_tokens(expression, &start, tokens, count_tokens);
    }
